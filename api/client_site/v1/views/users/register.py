...
async def register(
    ...
    if await register_limiter.is_blocked(email):
        logger.warning("Registration blocked due to too many attempts: %s", email)
    ...
    if existing_user and existing_user.is_verified:
        logger.warning("Registration failed—user already verified: %s", email)
        raise HTTPException(
    ...
    try:
        ...
        logger.error("Failed to send verification code to %s: %s", email, exc.detail)
        raise  # Propagate the HTTPException from the service

    # 6. Register this attempt for limiter purposes
    ...
    logger.info("Verification code dispatched to %s; user record ID %s", email, user.id)
