...
class SpeakingService:
    ...
    async def start_session(user, t: dict) -> Dict[str, Any]:
        ...
        price = await get_user_actual_test_price(user, TransactionType.TEST_SPEAKING)
        ...
        async with in_transaction():
            ...
            # Создаём шаблон теста
            test = await Speaking.create(
                title=f"IELTS Speaking Test {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M')}",
                description="Generated by AI"
            )

            # Создаём вопросы
            for part_key, part_num in [("part1", 1), ("part2", 2), ("part3", 3)]:
    ...
    async def get_session(session_id: int, user_id: int, t: dict) -> Dict[str, Any]:
        ...
        """
        session = await SpeakingSession.get_or_none(id=session_id, user_id=user_id)
        if not session:
            raise HTTPException(
        ...
        questions = await SpeakingQuestion.filter(test=session.test).order_by("part")
    ...
    async def restart_session(session_id: int, user_id: int, t: dict) -> dict:
        ...
        await SpeakingAnswer.filter(session=session).delete()
