...
class ReadingService:
    ...
    @staticmethod
    async def start_session(user_id: int, t: dict, level: str = "easy") -> Dict[str, Any]:
        """
                ...
                for q in passage_data.get("questions", []):
                    ...
                    if not q_type:
                        if isinstance(q.get("answers", None), list) and len(q["answers"]) == 0 and q.get("text"):
                            ...
                            raise HTTPException(status_code=500, detail=f"Некорректный вопрос: не удалось определить тип: {q}")
    ...
    @staticmethod
    async def get_session(session_id: int, user_id: int, t: dict) -> dict:
        """
        Get session details.
        """
        session = await Reading.get(id=session_id, user_id=user_id)
        passages = await session.passages.all().prefetch_related("questions")
        return {
            "id": session.id,
            "user_id": session.user_id,
